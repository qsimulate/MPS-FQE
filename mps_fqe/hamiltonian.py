...
class MPOHamiltonian(Hamiltonian):
    @classmethod
    def from_fqe_specific(cls, fqe_wf: FqeWavefunction,
                          fqe_ham: FqeHamiltonian,
                          pg="c1",
                          flat=False) -> "MPS":
        if not fqe_wf.conserve_number():
            raise TypeError('Wavefunction does not conserve number of particles.')
        if not fqe_wf.conserve_spin():
        ...
        if isinstance(fqe_ham, restricted_hamiltonian.RestrictedHamiltonian):
            ...
            fd = FCIDUMP(pg=pg,
                         n_sites=fqe_wf.norb(),
                         n_elec=fqe_wf._conserved['n'],
                         twos=fqe_wf._conserved['s_z'],
                         h1e=h1e,
                         g2e=g2e,
                         const_e=fqe_ham.e_0()
            )
    ...
    @classmethod
    def from_fqe_hamiltonian(cls, fqe_wf: FqeWavefunction,
                             fqe_ham: FqeHamiltonian,
                             pg="c1",
                             flat=False,
                             cutoff=1E-9) -> "MPS":
